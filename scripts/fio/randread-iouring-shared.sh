...
do
  ...
  sudo fio --name=$name --cpus_allowed_policy=split --rw=randread --direct=1 --ioengine=io_uring \
  --size=1G --gtod_reduce=0 --thread --group_reporting --time_based --runtime=60 --bs=4k --iodepth=128 \
  --cpus_allowed=0-15 --numjobs="$jobs" --filename=/mnt/$name.tmp --output "$results_dir"/"$name"."$jobs".txt
