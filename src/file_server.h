  ...
  // setup io_uring at the same time
    Status SetParameters(ServerContext *ctx, const Parameters *params, ACK *ack) {
        ...
        for (int i = 0; i < fn; i++) {
            ...
            fds[i] = open(my_test_file.data(), O_DIRECT | O_WRONLY | O_CREAT, 0644);
            if (fds[i] < 0) {
                perror("open");
            ...
            if (fstat(fds[i], &st)) {
                perror("fstat");
                return Status(StatusCode::UNKNOWN, "fstat failed");
            }

            if (st.st_size != buffer_size) {
                printf("resizing test file %s, original size is %luB, setting to %luB\n", my_test_file.data(), st.st_size, buffer_size);
                if (fallocate(fds[i], 0, 0, buffer_size)) {
                    perror("fallocate");
                    return Status(StatusCode::UNKNOWN, "fallocate failed");
                }
            }
        }
...
private:
    ...
    int N;
    int *fds;
    string test_file_prefix = "/mnt/test-file";
